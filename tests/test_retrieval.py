...
def test_s3_only_active_injected(tmp_path):
    ...
    items = retrieve(s, "orderkit", tmp_path, "Add a field to the Customer model", k=5)
    assert len(items) == 1
...
def test_retrieval_does_not_use_generator_family_id(tmp_path):
    ...
    hit = retrieve(s, "orderkit", ws, "Change the Invoice model to store float dollars")
    ...
    ws2 = tmp_path / "w2"
    ws2.mkdir()
    miss = retrieve(s, "orderkit", ws2, "Add a retry helper to the http client")
    assert miss == []
...
def test_scope_filter_blocks_other_repo(tmp_path):
    ...
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "models.py").write_text("# fixture", encoding="utf-8")
    assert retrieve(s, "another-repo", tmp_path, "Add a field to Customer") == []
