...
_SPECS = [
    ...
    _add_field_variant("GH5", "Invoice", "memo_code", "string", '""', '"M-9"', held_out=True),
    _rename_variant("GH6", "Invoice", "order_id", "linked_order_id", held_out=True),
    # — P2.1 GenB rev3:rename 主导校准批(armed family;机制 = 行为级 bypass)。
    #   参数域互不相交、与 GB/GH 不相交;Order.amount_cents 不 rename(pricing 按值依赖)。
    _rename_variant("GR1", "Customer", "name", "full_name", held_out=True),
    _rename_variant("GR2", "Customer", "tier", "account_tier", held_out=True),
    _rename_variant("GR3", "Invoice", "invoice_id", "invoice_ref", held_out=True),
    _rename_variant("GR4", "Invoice", "total_cents", "gross_cents", held_out=True),
    _rename_variant("GR5", "Shipment", "shipment_id", "shipment_uid", held_out=True),
    _rename_variant("GR6", "Shipment", "carrier", "carrier_label", held_out=True),
    _rename_variant("GR7", "Order", "order_id", "order_uid", held_out=True),
    _rename_variant("GR8", "Shipment", "order_id", "ship_order_id", held_out=True),
    _rename_variant("GR9", "Invoice", "order_id", "billed_order_id", held_out=True),
    _rename_variant("GR10", "Customer", "name", "customer_name", held_out=True),
    _rename_variant("GR11", "Shipment", "carrier", "shipping_carrier", held_out=True),
    _rename_variant("GR12", "Invoice", "invoice_id", "doc_id", held_out=True),
